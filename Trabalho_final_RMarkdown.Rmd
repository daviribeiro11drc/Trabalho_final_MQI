...
Adicionei a base de dados na pasta "dados",  dentro da pasta do repositório criado para o projeto.

Por fim, li a base de dados e transformei a aba "BASE_cargos" no objeto "dados":

```{r}
dados <- readxl::read_excel("dados/BASE_cargos2.xlsx", sheet = "BASE_cargos")
```
...
O intervalo de confiança, construído entre 0,09% e 13%, indica que é altamente provável que a diferença real entre as proporções esteja nesse intervalo, para o nível de significância de 95%. Dado que o intervalo não inclui o zero, há evidência estatisticamente significativa de que as proporções são diferentes.